        ...
        if not a.is_empty():
            ...
            for digit in range(passes):
                ...
                while curr is not None:
                    ...
                    index = curr._value % n // d 
                    #Add the number to the proper bucket
                    counter = 0
                    curr_bucket = buckets._front
                    while counter < index:
                        curr_bucket = curr_bucket._next
                        counter += 1
                    curr_bucket._value.append(curr._value)
                    curr = curr._next
                #put the values back into the original array
                index = 0
                bucket = buckets._front
                while bucket is not None:
                    while bucket._value._front is not None:
                        a.insert(index, bucket._value.remove_front())
                        index += 1
                    bucket = bucket._next
            a.clean()
