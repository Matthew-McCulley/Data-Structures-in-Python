...
class List:
    ...
    def insert(self, i, value):
        ...
        if i < -self._count:
            ...
            while curr is not None and n != i:
                prev = curr 
                curr = curr._next 
                n += 1
    ...
    def reverse_r(self):
        ...
        current = self._front
        temp = current
        current,previous,temp = self._reverse_r_aux(current, previous, temp)
        return
    def _reverse_r_aux(self, current, previous, temp):
        if current is not None:
            temp = current._next
            current._next = previous
            previous = current
            current = temp
            self._reverse_r_aux(current, previous, temp)
        else:
            self._front = previous
        return current, previous, temp
    ...
    def is_identical_r(self, other):
        ...
        if self._count != other._count:
            ...
            source_node = self._front 
            other_node = other._front
            identical = self._identical_r_aux(source_node, other_node)
            
        return identical
    
    def _identical_r_aux(self, source_node, other_node):
        if source_node is not None and source_node._value == source_node._value:
            identical = self._identical_r_aux(source_node._next, other_node)
    ...
    def _move_front_to_rear(self,source):        
        ...
        if source._front == None:
            source._rear = None
        if self._count == 0:
            self._front = _List_Node(temp._value, None) 
            self._rear = self._front
        else:
            self._rear._next = _List_Node(temp._value, None)
            self._rear = self._rear._next
        self._count += 1
        return 
    ...
    def _linear_search_r(self, key):
        ...
        index = -1
        i = 0
        previous, current, index = self._linear_search_aux(previous, current, index, i, key)
        return previous, current, index
    
    def _linear_search_aux(self, previous, current,index, i, key):
        if current is not None and current._value != key:
            previous, current, index = self._linear_search_aux(current, current._next,index, i+1, key)
    ...
    def intersection_r(self, source1, source2):
        ...
        source1_node = source1._front 
        self._intersection_r__aux(source1_node, source2)
        return
    
    def _intersection_r__aux(self, source1_node, source2):
        if source1_node is not None:
            ...
            self._intersection_r__aux(source1_node._next, source2) 
    ...
    def union(self, source1, source2):
        ...
        #looping through source 1 list
        while source1_node is not None:
            value = source1_node._value 
            
            _,current,_ = self._linear_search(value)
            #if value is not already in the self list
            if current is None:
                self.append(value)
            source1_node = source1_node._next 
    ...
    def combine(self, source1, source2):
        ...
        #while both of the sources are not empty
